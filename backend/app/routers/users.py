...
class UserResponse(BaseModel):
    id: uuid.UUID
    organization_id: Optional[uuid.UUID] = None  # Fixed typo from organization_j_id
...
def update_user_profile(
    ...
    """Update personal credentials, name, email, and phone directly in PostgreSQL (aligned with account.html)."""
    user = parse_user_identifier(db, user_id, organization_id)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
